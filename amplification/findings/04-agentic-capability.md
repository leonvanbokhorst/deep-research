...
**The ambient channel is under-appreciated.** *Science Advances* (2026, N = 2,582) showed a **biased AI writing assistant** shifted users' expressed attitudes toward its own position, that most were **unaware of the bias**, and that the effect exceeded the same suggestions shown as static text.
...
The closest artefact is **XARELLO** (WASSA@ACL 2024), a reinforcement-learning attacker that "learns from previous successes and failures," reaching a 100% confusion score against a BiLSTM checker with 77% text quality — and notably performing *better* against a larger model (GEMMA-2B). But it is **offline**, attacks *classifier robustness on a fixed dataset*, and its target is credibility assessment, not a live verdict on its own fresh claim. OpenAI's own assessment is "evolution rather than revolution"; the observed AI benefit is avoiding "grammatical errors that once made them easy to spot," not reading fact-checks.

The defensive side is now quantified, and it points at **latency, not evasion**. Roitero et al., *Nature Communications*, May 2026: across **237,180 annotated cascades and >431M reposts over 20 months**, displaying a Community Note produced a **61.2% reduction in reposting (DiD ATT = −0.612)** and a **94.3% increase in the odds the author deleted the post**. But notes appeared at an average lag of **62.9 hours (median 18.1)** against a post half-life of **6.25 hours**, yielding a **system-wide effect of only 14.9% (median 10.1%)**. At a two-hour lag it would have been **52.3%**; after 24 hours the effect is statistically insignificant, and it is **significantly weaker for high-follower, verified and political accounts**. Independent monitoring of the same period found only **10% of high-engagement false claims** about Hurricanes Helene and Milton received a visible Note, at an average delay of **46 hours**.
...
- **AI-text detection is not a defence** — unreliable, false-positive-prone against human writers (one Stanford suite flagged **61% of human TOEFL essays**), and largely irrelevant because networks are connected structurally. **Platform detection does work where it is behavioural**: Reddit reports ~**23M spam views/day blocked before exposure**, ~**2M fake votes/day revoked**, enforcement in under five seconds, and a **40% reduction in false positives** — the rare published evidence that integrity systems can improve on both axes.
