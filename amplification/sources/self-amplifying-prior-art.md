...
**MITRE ATLAS** (https://raw.githubusercontent.com/mitre-atlas/atlas-data/main/dist/ATLAS.yaml) contains **no** technique named or described with "amplification," "reflection," "reflexive" or "cascade." Its nearest entries are **AML.T0061 "LLM Prompt Self-Replication"** (a prompt that makes the model replicate and persist itself) and **AML.T0060 "Publish Hallucinated Entities"** (**H**). The established AI threat taxonomy therefore does **not** name the small-action / disproportionate-target-reaction pattern. OWASP LLM Top 10 and NIST AI 100-2 were **not** checked at source: **UNVERIFIED**.
...
**Already named.** (1) *Deliberately making the target generate its own harm*: Liddell Hart (**H**), reflexive control (**H**), cost imposition (**M**), strategy of tension (**H** for the term, **M** for the Gladio attribution). (2) *The target's reaction is where the harm lives*: Merton (**H**) is the best single borrow; Mueller's "costs of reaction" is the security-specific quantified version (**H**). (3) *Small input → large response, weaponised*: "amplification attack" is canonical (**H**), and the closest true target-as-amplifier instance is the authentication **reflection attack** (**H**). (4) *Provocation as method*: *agent provocateur*, *provokatsiya*, false flag, propaganda of the deed (**M**).
